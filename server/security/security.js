...
app.all(properties.api + '/*', function(req, res, next) {
    ...
    if (!roles) {
        ...
        // Verify JWT token
        let token = req.headers.authorization && req.headers.authorization.replace("Baerer ", "");
...
app.post(properties.api + '/changePassword', function(req, res) {
    ...
    db_Manage_Film_Example_db.User.findOne({ username: req.user.username, password: req.body.passwordOld }).exec(function(err, user) {
...
var getUser = function(username, password, callback) {
    ...
    db_Manage_Film_Example_db.User.findOne({ username: username, password: password }).lean().exec(function(err, user) {
        callback(user);
    });
};
...
var createUser = function() {
    ...
    db_Manage_Film_Example_db = require('../db/Manage_Film_Example_db_schema.js');
    db_Manage_Film_Example_db.User.count().exec(function(err, count) {
        if (count == 0) {
            var admin = new db_Manage_Film_Example_db.User({
                username: 'admin',
